...
EvolutionaryModel::EvolutionaryModel(Alignment* a, RandomVariable* r) : activeTree(0), aln(a), rng(r) {
...
double EvolutionaryModel::lnLikelihood(){

    std::vector<Node*>&  dpSeq = tree[activeTree]->getPostOrderSeq();
    for(Node* n : dpSeq){
        //Only update the conditional likelihoods if the node has changed
        if(n->getNeedsUpdate() == true){
            ...
            n->setNeedsUpdate(false);
        }
    }
    
    //Calculate the likelihood of the tree by summing up the likelihood at the root.
    double* pR = (*condL)(tree[activeTree]->getRoot()->getIndex(), tree[activeTree]->getRoot()->getActiveCL());
